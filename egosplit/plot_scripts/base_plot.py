...
# Create a plot
def make_plot(data, graphs, algo_matches='', xlabel=None, remove_algo_part='',
              file_name='', title='',
              plot_args=None, ax_set=None, plot_type=PlotType.line, add_algos=None, x=None,
              hue=None, y=None, one_plot_per_graph=False):
	graph_filtered_data = data.query('graph.str.contains(@graphs)').copy()
	# Get a list of all algorithms
	algo_set = set()
	if algo_matches is not None:
		assert(isinstance(algo_matches, list))
		for algo_match in algo_matches:
			algo_data = graph_filtered_data.query('algo.str.contains(@algo_match)')
			algo_set = algo_set.union(set(algo_data.groupby('algo').mean().index.values))
	else:
		algo_set = set()
	if add_algos:
	...
	# Filter data
	graph_filtered_data.query('algo in @algo_list', inplace=True)
	if len(graph_filtered_data) is 0:
		return
	...
	if plot_args['x'] != 'graph' or one_plot_per_graph:
		graph_list = graph_filtered_data.groupby('graph').mean().index.values
	else:
		graph_list = [graphs]
	...
	for graph in graph_list:
		if len(graph_list) == 1:
			graph_data = graph_filtered_data
		else:
			graph_data = graph_filtered_data.query('graph == @graph')
		...
		if type(algo_matches) == str and remove_algo_part == '':
			remove_algo_part = algo_matches
