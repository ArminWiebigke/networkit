...
void oslom_net_evaluate::set_changendi_cum() {
	if (dim != 0 && total_stubs != 0) {
		int flat_until = cast_int(total_stubs / dim * 3);
 ...
 */
void oslom_net_evaluate::erase_from_cgroup(int wnode) {
	auto itm = cgroup.labels_to_facts.find(wnode);
	if (itm != cgroup.labels_to_facts.end()) {
		int internal_degree = itm->second.internal_degree;
		int node_degree = itm->second.degree;
		double mtlw = itm->second.internal_edgeweight;

		kin_cgroup -= 2 * internal_degree;
		ktot_cgroup -= node_degree;
		int kout_g = ktot_cgroup - kin_cgroup;
		int tm = total_stubs - ktot_cgroup;

		double fi = Stochastics::compute_global_fitness_ofive(internal_degree, kout_g, tm,
		                                                      node_degree, mtlw,
		                                                      neighs.size() + 1,
		                                                      dim - cgroup.size() + 1);
		neighs.insert_node(wnode, internal_degree, node_degree, mtlw, fi);
		cgroup.erase_node(wnode);
...
bool oslom_net_evaluate::erase_the_worst(int &wnode) {
	...
	erase_from_cgroup(wnode);
	return true;
}

bool oslom_net_evaluate::erase_the_worst() {
	int wnode;
	return erase_the_worst(wnode);
}
 ...
 */
void oslom_net_evaluate::insert_into_cgroup(int node) {
	int internal_degree, node_degree;
	double mtlw;
	{
		auto itm = neighs.labels_to_facts.find(node);
		if (itm != neighs.labels_to_facts.end()) {
			internal_degree = itm->second.internal_degree;
			node_degree = itm->second.degree;
			mtlw = itm->second.internal_edgeweight;
		} else {
			internal_degree = 0;
			node_degree = vertices[node]->stub_number;
			mtlw = 0;
		}
	}
	...
	int kout_g = ktot_cgroup - kin_cgroup;
	int tm = total_stubs - ktot_cgroup;
	double fi = Stochastics::compute_global_fitness_ofive(internal_degree, kout_g, tm, node_degree,
	                                                      mtlw, neighs.size(),
	                                                      dim - cgroup.size());
	kin_cgroup += 2 * internal_degree;
	ktot_cgroup += node_degree;
	kout_g = ktot_cgroup - kin_cgroup;
	tm = total_stubs - ktot_cgroup;

	cgroup.insert_node(node, internal_degree, node_degree, mtlw, fi);
	neighs.erase_node(node);

	std::deque<int> tobe;
	for (int i = 0; i < vertices[node]->links->size(); i++) {
		if (!cgroup.update_group(vertices[node]->links->l[i],
		                         vertices[node]->links->w[i].first, // Number of edges to this node
		                         vertices[node]->links->w[i].second, // Sum of edgeweight
		                         dim - cgroup.size(), neighs.size(), kout_g, tm,
		                         vertices[vertices[node]->links->l[i]]->stub_number,
		                         tobe))
			neighs.update_neighs(vertices[node]->links->l[i],
			                     vertices[node]->links->w[i].first,
			                     vertices[node]->links->w[i].second,
			                     dim - cgroup.size(), kout_g, tm,
			                     vertices[vertices[node]->links->l[i]]->stub_number);
	}
}
...
/**
 * Run Clean Up Procedure with the neighbors, then run it only with the nodes in the group.
 * @param group_to_clean
 * @param cleaned_group
 * @param number_of_runs
 * @return
 */
double oslom_net_evaluate::CUP_both(const std::deque<int> &group_to_clean,
                                    std::deque<int> &cleaned_group, int number_of_runs) {
	if (number_of_runs == -1)
		number_of_runs = paras->clean_up_runs;
	WeightedTabdeg previous_tab_c;
	WeightedTabdeg previous_tab_n;
	...
	initialize_for_evaluation(group_to_clean, previous_tab_c, previous_tab_n, kin_cgroup_prev,
	                          ktot_cgroup_prev);
	bscore = CUP_runs(previous_tab_c, previous_tab_n, kin_cgroup_prev, ktot_cgroup_prev,
	                  cleaned_group, false, number_of_runs);

	initialize_for_evaluation(cleaned_group, previous_tab_c, previous_tab_n, kin_cgroup_prev,
	                          ktot_cgroup_prev);
	bscore = CUP_runs(previous_tab_c, previous_tab_n, kin_cgroup_prev, ktot_cgroup_prev,
	                  cleaned_group,
	                  true, /*this "true" means I can only look at nodes in previous_tab_c */
...
void oslom_net_evaluate::get_external_scores(WeightedTabdeg &neighbors,
	...
	while (bit != neighbors.fitness_to_label.end()) {
...
double oslom_net_evaluate::CUP_from_list_simple(CupDataStruct &a, std::deque<int> &gr_cleaned) {
	int Nstar; // Number of external nodes
	if (paras->weighted)
...
/**
 * Initialize the data structures for the evaluation for a given group.
 * @param eval_group The group that will be evaluated later.
 * @param previous_tab_c Data structure for the current group.
 * @param previous_tab_n Data structure for the current neighbors.
 * @param kin_cgroup_prev
 * @param ktot_cgroup_prev
 */
void oslom_net_evaluate::initialize_for_evaluation(const std::deque<int> &eval_group,
                                                   WeightedTabdeg &previous_tab_c,
                                                   WeightedTabdeg &previous_tab_n,
                                                   int &kin_cgroup_prev,
                                                   int &ktot_cgroup_prev) {
	set_cgroup_and_neighs(eval_group);
