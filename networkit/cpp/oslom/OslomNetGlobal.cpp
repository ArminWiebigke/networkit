...
int OslomNetGlobal::try_to_merge_discarded(IntMatrix &discarded,
    ...
    for (auto &merged_modules : M_raw) {
        if (merged_modules.size() > 1) {
            ...
            std::cout << "merged " << merged_modules.size() << " discarded modules "<< std::endl;
//            print_id(merged_group, std::cout);
            ...
            try_add_good_group(cleaned_group, b_score, merged_group, good_modules, bscores_good,
            		new_discarded);
        }
    }
    return 0;
 ...
 * @return
 */
void OslomNetGlobal::try_add_good_group(
...
OslomNetGlobal::clean_up(const std::vector<std::deque<int>> &modules, int upper_node_id) {
    ...
    for (int gr_id = 0; gr_id < int(modules.size()); gr_id++) {
        std::deque<int> group;
