...
void SignificanceCommunityCleanUp::checkMergedCommunities() {
	for (const auto &communities : mergedCommunities.getSubsets()) {
		...
		std::cout << "Merged: " << mergedCommunity.size() << " -> " << cleanedCommunity.size() << std::endl;
		if (cleanedCommunity.empty())
...
void SignificanceCommunityCleanUp::createDiscardedCommunitiesGraph() {
...
void SignificanceCommunityCleanUp::localOptimization() {
...
bool SignificanceCommunityCleanUp::tryLocalMove(node u) {
